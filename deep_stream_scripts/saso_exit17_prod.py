    ...
    while l_frame:
        ...
        while l_user:
            try:
                ...
                if user_meta.base_meta.meta_type == pyds.nvds_get_user_meta_type("NVIDIA.DSANALYTICSFRAME.USER_META"):
                    ...
                    common_vars.lane_stage_details[lane_no][stage]["vehicle_in_roi"]=0
        ...
        while l_obj:
            ...
            if utils.is_point_inside_polygon([obj_meta.rect_params.left+obj_meta.rect_params.width/2 ,\
                                                obj_meta.rect_params.top+obj_meta.rect_params.height/2],common_vars.roi_coordinates[str(lane_no)]) and (obj_meta.class_id == common_vars.PGIE_CLASS_ID_VEHICLE or obj_meta.class_id == common_vars.PGIE_CLASS_ID_TRUCK or \
                                                obj_meta.class_id == common_vars.PGIE_CLASS_ID_TOP_CAR or obj_meta.class_id == common_vars.PGIE_CLASS_ID_TOP_TRUCK):                                                                                                        
            ...
            if obj_meta.class_id == common_vars.PGIE_CLASS_ID_VEHICLE or \
                ...
                if "entry_exit_360_update" in common_vars.input_config[str(frame_meta.pad_index)]["functionName"] :
                    ...
                    if obj_meta.class_id == common_vars.PGIE_CLASS_ID_PERSON:
                ...
                try:
                    ...
                    if user_meta.base_meta.meta_type == pyds.nvds_get_user_meta_type("NVIDIA.DSANALYTICSOBJ.USER_META"):   
                        ...
                        if user_meta_data.roiStatus and \
                            ...
                            ub_points[obj_meta.object_id] = [[obj_meta.rect_params.top,obj_meta.rect_params.left, \
                                obj_meta.rect_params.width,obj_meta.rect_params.height],obj_meta.class_id,user_meta_data.roiStatus]
        ...
        # Condition to check the stage
        if stage in ["ubtopfront", "ubtopback", "ubbottomfront", "ubbottomback"]:
            check_utils.ub_check(lp_objects, ub_points, int(lane_no), stage)
        else:
        ...
        if "top_360" in common_vars.input_config[str(frame_meta.pad_index)]["name"]:
            ...
            # check_utils.carbon_check_exit17()
        ...
        bbox_metadata["frameHeight"] = frame_meta.source_frame_height
        bbox_metadata["frameWidth"] = frame_meta.source_frame_width
...
def decodebin_child_added(child_proxy,Object,name,user_data):
    common_logger.debug(f"Decodebin child added: {name}\n")
    print("####3####",name)
    ...
    if "source" in name:
        ...
        common_logger.debug("Setting rtspsrc protocol to TCP (interleaved)")
        #Object.set_property("select-rtp-protocol", 4)  # Set protocol to TCP interleaved
        Object.set_property("latency", 500)  # Optionally set latency
        # Object.set_property("rtsp-reconnect-attempts", -1) # Set to -1 for infinite reconnect attempts

    # Check if the child is 'rtspsrc' and set the TCP protocol
    if "rtspsrc" in name:
        print("----------------------",name)
        common_logger.debug("Setting rtspsrc protocol to TCP (interleaved)")
        Object.set_property("select-rtp-protocol", 4)  # Set protocol to TCP interleaved
...
def create_source_bin(index,uri):
    common_logger.debug("Creating source bin")

    # Create a source GstBin to abstract this bin's content from the rest of the
    # pipeline
    bin_name="source-bin-%02d" %index
    common_logger.debug(bin_name)
...
def main(args):
    ...
    for i in range(number_sources):
        common_logger.debug("Creating source_bin ",i," \n ")
        uri_name=args[i+1] 
    ...
    nvanalytics.set_property("config-file", "../configs/saso_test_config.txt")
